...
export const WelcomePage: FC = () => {
  ...
  return (
    <Box
      className={`flex flex-col  w-full relative z-10 bg-cover bg-center h-screen before:content-['']
    ...
    >
      ...
      >
        ...
        >
          ...
          <Typography
            className='font-Rowdies font-bold text-[16px] text-YellowSecond tracking-[.5px] text-stroke-1 text-stroke-Black'
            text={Title}
          />
          <Typography
        ...
        >
          {Actions.map(item => (
            <Button
              key={item.id}
              className={`w-[158.17px] h-[50px] rounded-[20px] flex gap-2 items-center justify-center ${item.bgColor} border-2 border-solid border-white`}
              icon={item.icon}
              content={item.text}
            />
          ))}
      ...
      <Button
        className='w-[330px] h-[50px] mx-auto mt-[45px] font-Rowdies flex items-center justify-center bg-YellowSecond shadow-YellowSecond border-2 border-solid border-White rounded-[20px] text-BluePrimary'
